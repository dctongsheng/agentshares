...
This saves credentials to `~/.agent-share-config.json` and a session cookie to `~/.agent-share-cookies.txt`. The base URL defaults to `https://agentshare.app/` and can be auto-read from the project's `.env` file (`NEXTAUTH_URL`).
